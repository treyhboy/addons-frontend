...
describe(__filename, () => {
  ...
  const renderWithExperiment = ({
    ...
    // See: https://github.com/airbnb/enzyme/issues/1647
    //
    // 1. Render everything
  ...
  it('creates a cookie upon construction if none has been get', () => {
    const id = 'hero';
    const cookies = fakeCookies({
    ...
    sinon.assert.calledWith(cookies.set, `experiment_${id}`);
  });

  it('does not create a cookie upon construction if one has been get', () => {
    const id = 'hero';
    const cookies = fakeCookies({
      get: sinon.stub().returns(`experiment_${id}`),
    });
